...
const selectNodeColumns = `
  ...
  node_version, capabilities, codex_installations, desired_app_server,
...
export async function registerNode(pool, nodeId, body) {
  ...
  const result = await pool.query(
    `UPDATE codex_nodes SET
       ...
       node_version = $7, capabilities = $8::jsonb,
       codex_installations = $9::jsonb, last_seen_at = NOW(), updated_at = NOW()
    ...
    [nodeId, body.nodeKey, body.hostname, body.platform, body.architecture, body.nodeMode,
      nodeVersion, JSON.stringify(capabilities), JSON.stringify(codexInstallations)],
  );
