...
const zero = [0, 0, 0, 0];
export function newCostProjection(awaitForkBoundary = false) {
...
export function applyCostRecord(state, record, threadId = null) {
  ...
  if (payload?.type === "thread_settings_applied") {
    ...
    if (state.awaitForkBoundary && !state.scopeStarted && payload.thread_id === threadId) state.scopeStarted = true;
    return;
  }
  ...
  if (!total) { if (state.scopeStarted) incomplete(state, "invalid_usage"); return; }
  state.observed = true;
  ...
  if (!job) {
    job = (async () => {
      ...
      while (cursor < thread.itemCount) {
        ...
        for (const row of result.rows) applyCostRecord(entry.state, row.payload, thread.threadId);
