...
import { addThreadTokenUsage } from "./thread-token-usage.mjs";
...
export async function listImportedThreads(pool, storeId = defaultStoreId, limit = 200, threadId = null, archived = null) {
  ...
  return addThreadTokenUsage(pool, id, await addThreadReadStates(pool, id, await addThreadActivities(pool, id, result.rows.map((row) => ({
    ...
    itemCount: Number(row.item_count), generation: Number(row.active_generation), tokenUsage: row.token_usage,
