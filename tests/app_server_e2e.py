...
WORKSPACE = str(PROJECT_DIRECTORY / "tests" / "workspace")
CLIENT_A = str(PROJECT_DIRECTORY / "tests" / "client-a")
CLIENT_B = str(PROJECT_DIRECTORY / "tests" / "client-b")
...
def wait_remote_store_contains(settings, marker, timeout=30):
    ...
    token = settings.get("bearer_token") or os.environ.get("MIRA_NODE_TOKEN")
    if not token:
        raise RuntimeError("MIRA_NODE_TOKEN is required for the remote ThreadStore test")
...
def main():
    for directory in (WORKSPACE, CLIENT_A, CLIENT_B):
