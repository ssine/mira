...
try {
  ...
  const details = page.locator('#conversationDetails');
  ...
  const detailsLeft = await details.evaluate(element=>element.getBoundingClientRect().left);
  assert.ok(Math.abs(detailsLeft-(await page.evaluate(()=>innerWidth)-60))<=2,'details follows the opening finger');
  await page.waitForTimeout(200); await endDrag();
  await page.waitForFunction(()=>!document.querySelector('#conversationDetails').open);
  await swipe(300,300,-190,0,{cancel:true});
  await page.waitForFunction(()=>!document.querySelector('#conversationDetails').open);
