...
1.0.42 在会话左上角 Mira 菜单新增侧边栏布局：自动、常驻、抽屉。竖屏和窄窗口可手动保持并排显示，布局与宽度在当前浏览器中记忆；手机窄屏临时使用抽屉，窗口变宽后恢复常驻偏好。

1.0.38 加快已提交历史上传的暂存清理：按条数和存储字节分批回收，有积压时持续推进，并轮转处理不同上传。正式历史与提交重试凭据保持独立；修复 Windows runtime 验收测试遗留子进程导致发布失败的问题。

1.0.34 将主会话与子 Agent 分开分页，保留完整项目入口，并在展开时加载子会话。
父会话 Token 与金额汇总全部子孙会话，包含归档且排除复制的分支历史；列表刷新保留已加载记录。

1.0.32 减少大对话恢复时重复读取子对话元数据的开销，显示恢复进度、等待时间和取消按钮，
并保留慢请求的确认，避免超时后残留“请求未完成”状态。Codex 打包会拒绝开发构建，
确保中断的工具调用可以按上游正式构建的规则恢复。

1.0.29 扩大费用投影的有界缓存，并优先保留最近使用的会话，避免较大的子 Agent 树在轮询时
反复淘汰、重算历史费用。子会话追加和换代仍按各自的历史版本更新，计价规则不变。

1.0.28 进一步降低会话列表的数据库 CPU 开销：一次提取会话元数据，避免逐字段重复解压同一份 JSONB。
会话标题、时间排序、运行绑定和已读状态继续实时读取，无需数据库迁移。

1.0.26 修复重新打开运行中对话后停止按钮不可用的问题，停止操作直接连接该对话的运行 Node 和账号。
会话列表复用未变化的历史摘要，避免轮询反复扫描大段 JSON；节点在线状态和共享已读位置仍实时刷新。
...
1.0.22 修复上下文兼容处理后旧错误提示重新出现的问题。确认最新错误后，重新打开对话也不会
要求重复处理；该账号再次发生新的兼容错误时仍会提示。

1.0.21 从主会话切换账号时，将整棵子 Agent 树一同交接；已存在的子 Agent 可直接用新账号继续，
保留各自模型和原始历史。需要配套 Codex runtime `0.153.1-mira.10`。
...
1.0.12 为大会话分支提供分块历史上传、实时进度和取消操作，避免整段历史超过请求大小限制；
配套 Codex runtime 升至 `0.153.1-mira.8`，历史仍在 PostgreSQL 中原子发布。

1.0.11 按对话在浏览器保存未发送的文字与附件，刷新或关闭重开后恢复草稿；同时修复历史图片展示、
工具详情滚动位置，以及未设置运行时目录时 Linux 用户服务的发现。

1.0.10 让低频目录组检查同时支持 NixOS 的系统 `id` 路径，确保组变更提示在
...
```sh
curl -fsSL https://raw.githubusercontent.com/ssine/mira/main/scripts/install.sh | sh -s -- --role node --server https://mira.example.com --version 1.0.42
```

Windows x64（在管理员 PowerShell 中运行）：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/ssine/mira/main/scripts/install.ps1'))) -Role node -Server 'https://mira.example.com' -Version '1.0.42'"
```
...
显示字节进度并支持取消（取消不会发出消息，会清理本次暂存文件）。未发送的文字与附件按对话保存到
当前浏览器的 IndexedDB；刷新或关闭后重新打开可恢复，新对话草稿按项目区分。发送成功清理已发送内容，
失败或取消上传保留草稿。草稿不跨浏览器或设备同步，清理网站数据会移除草稿；存储空间不足等失败会在输入框下提示。

发送时，附件暂存于实际运行节点的系统临时目录（受限根配置下回退到 thread 工作目录）；它不是跨节点同步盘。
...
项目列表按持久化的父子关系，将 subagent 放到各自主线程下的“子 Agent”折叠列表中，支持多级展开。
子线程不占项目默认四个近期主线程的名额；折叠栏汇总下级数量、运行、失败和未读状态，新建子线程会自动出现。
展开状态在当前页面的列表刷新中保留，直接打开子线程链接会展开其父级路径。不同运行节点或工作目录的
子线程仍跟随主线程分组，但保留自己的执行信息和独立历史。父线程未在当前列表中（例如已归档或超出最近
300 条）时，子线程保留独立入口；普通 fork 继续作为独立对话显示。
...
| `GET` | `/v1/codex/threads/{id}/transcript` | 管理员按页读取可重建的 Web 消息、工具与历史图片轨迹 |
| `GET` | `/v1/codex/threads/{id}/transcript/image` | 管理员按 generation、记录序号和内容位置读取历史图片，不回读本地路径 |
