...
1.0.15 加入七日额度消耗参考线，并从会话历史恢复推理强度设置；同时包含大会话分块 fork 修复，
...
```sh
curl -fsSL https://raw.githubusercontent.com/ssine/mira/main/scripts/install.sh | sh -s -- --role node --server https://mira.example.com --version 1.0.15
```

Windows x64（在管理员 PowerShell 中运行）：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/ssine/mira/main/scripts/install.ps1'))) -Role node -Server 'https://mira.example.com' -Version '1.0.15'"
```
