...
workspace identity in the account response. History reads span runtime hashes on the same Node,
so changing the binary path during an upgrade preserves the latest account's earlier observations.
Samples for other accounts become gaps; returning to an account restores its earlier observations.
The stored runtime hashes and original samples remain unchanged.
...
Node, installation, account and range, cleared on logout. Empty results expire after 30 seconds;
explicit range selection always revalidates, and aborted requests cannot overwrite newer responses.
Existing live account reads keep their own five-minute cache.
