...
Node 的通知缓存不作为停止依据：停止或替换运行实例前，先阻止新的执行请求，再分页核验
App Server 中全部已加载对话的状态。漏收完成通知不会让空闲账号永久无法停止；实际运行的
子任务、未确认的执行请求、凭据操作和对话交接仍阻止停止。查询失败或状态变化时保留实例并
报告原因；进程退出后清理其执行标记，旧实例的迟到通知不能污染新实例。
浏览器或反向通道断开时，已发送的本地执行请求仍短暂排空响应；超时或本地连接故障时保留
未确认状态，不把断线当成请求完成。
...
未登录、未配置且没有额度的自动生成“默认账号”仅作为兼容绑定保留，不出现在余额列表中；
有登录快照、配置或凭据变更记录的默认账号以及用户命名的离线账号仍然显示。

未提供额度时，侧栏直接显示最近七天的估算费用，同时最多读取两个账号；手机在对话页面
可见时即预取费用，收起侧栏不取消该读取，实时 Node 账户订阅仍只在侧栏展开时启用。详情
...
历史请求缺少执行事件时，读取当时的 session_meta/thread_settings_applied provider，仅在它
与现有账号名称及 Node 报告的 provider 别名具有唯一对应关系时回补归属，并标记为部分估算。
精确执行事件始终优先；多个账号共用的 provider 不猜测归属。原生记录缺少 rollout timestamp
时使用数据库记录时间，导入历史优先恢复原始来源时间；来源也缺少时间时不计入导入当天。
没有可确定归属、有效请求时间或价格的历史不能作为完整账单回填；空记录保持未知，部分估算
保留标记。它不表示套餐扣减或自定义服务商的实际账单。后台按页增量解析历史，接口从派生明细按小时
聚合；回填进度与计价完整性分别报告，不改写历史。

费用由后台增量投影提供，详情见 [费用统计架构](account-cost-projection.md)。Schema 31 新增
可丢弃、可重建的 PostgreSQL 请求费用明细与每个会话的解析检查点。单个后台 worker 按公平
顺序处理会话，每页最多 1024 条计费相关原始记录，明细和游标原子提交；重启、取消和丢失
提交响应不重算已完成页。generation、fork 来源、导入来源和解析/价格版本改变时重建。
费用 HTTP 请求不再读取 canonical 正文，只聚合按时间索引的费用明细；账号归属仍按实时
执行元数据解析，迟到执行事件、重命名和 provider 唯一性变化不要求重读原始历史。

Server 按账号、范围、时区和本地日期合并并缓存查询，最多 64 项、两次并发，正常一分钟，
后台未追平时五秒。过期金额先返回并刷新，查询失败保留旧结果并三十秒重试。响应保留
`cache` 新鲜度契约，并增加全局 `projection` 进度及失败会话数。回填中的金额标记部分估算，
空结果保持未知；网页显示后台汇总进度。删除 canonical 会话级投影会级联删除费用派生表，
旧 Server 可以忽略新表，既有 schema 和 canonical 提交协议不变。
