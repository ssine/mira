...
费用接口在 Server 进程内按账号名、范围、时区和本地日历日期共享五分钟缓存，最多保留
64 个结果、同时执行两次统计，相同查询的并发请求合并。单个客户端断开不取消共享计算，
Server 关闭时统一取消。过期结果立即返回并后台刷新，失败保留上次金额、间隔三十秒重试；
冷缓存仍需等待首次计算。响应的 `cache` 提供 `updatedAt`、`expiresAt`、`stale` 和
`refreshing`，页面按服务端到期时间更新，并标注旧结果及统计时间。缓存只是在内存中的
可重建投影，进程重启后重新计算。Schema 30 为费用事件和历史 provider 事件增加部分索引，
避免按账号统计时扫描整段会话正文与工具输出；不改变既有历史或前一版本的 SQL 契约。
