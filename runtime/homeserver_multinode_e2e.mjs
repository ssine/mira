...
const controlUrl = (process.env.CONTROL_SERVER_URL ?? "http://127.0.0.1:18787").replace(/\/$/, "");
...
try {
  homeNode = (await nodes()).find((node) => node.nodeKey === "homeserver-docker");
  if (!homeNode) throw new Error("homeserver-docker node is not registered");
  ...
  const writableRoot = roots.roots.find((root) => root === "/home/node") ?? roots.roots[0];
  const remoteFile = `${writableRoot}/node-agent-e2e-${process.pid}.txt`;
  ...
  wslAgent = spawn("node", ["node-agent/agent.mjs"], {
    cwd: projectDirectory,
    env: {
      ...process.env,
      CONTROL_SERVER_URL: controlUrl,
      CONTROL_SERVER_TOKEN: token,
      NODE_AGENT_KEY: wslNodeKey,
  ...
  for (const stream of [wslAgent.stdout, wslAgent.stderr]) {
    stream.setEncoding("utf8");
    stream.on("data", (chunk) => agentOutput.push(chunk));
  }

  const wslNode = await waitFor(async () => {
    if (wslAgent.exitCode !== null) {
      throw new Error(`WSL node agent exited with ${wslAgent.exitCode}: ${agentOutput.join("").slice(-2_000)}`);
    }
    const node = (await nodes()).find((item) => item.nodeKey === wslNodeKey);
    return node?.status === "online" && node.reportedAppServer?.status === "running" ? node : null;
  }, "WSL node registration and App Server startup");
  wslNodeId = wslNode.nodeId;

  const client = await openAppServer();
  let threadId;
  try {
    const started = await client.call("thread/start", {
      cwd: `${projectDirectory}/runtime/workspace`,
  ...
  if (wslAgent?.exitCode === null) {
    const exited = new Promise((resolve) => wslAgent.once("exit", resolve));
    wslAgent.kill("SIGTERM");
    await exited;
  }
  if (homeNode) {
    await setDesired(homeNode.nodeId, {
