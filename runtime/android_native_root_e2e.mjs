...
const token = process.env.CONTROL_SERVER_TOKEN;
const nodeKey = process.env.ANDROID_NATIVE_NODE_KEY ?? "mira-android-root";

if (!token) throw new Error("CONTROL_SERVER_TOKEN is required");
...
try {
  ...
  if (status.native?.uid !== 0 || status.rootEnabled !== true) {
    throw new Error(`Android agent is not root: ${JSON.stringify(status.native)}`);
  }
