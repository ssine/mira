...
final class AndroidBridgeServer {
    ...
    AndroidBridgeServer(Context context, ScreenCaptureController capture, String token) {
    ...
    int start() throws Exception {
        ...
        acceptThread = new Thread(this::acceptLoop, "mira-android-bridge");
        acceptThread.start();
        ...
        while (server != null && !server.isClosed()) {
            try {
                ...
                if (server != null && !server.isClosed()) {
                    MiraNodeService.publishStatus(context, "Android bridge failed: " + error.getMessage(), null);
                }
            }
        }
    }
    ...
    private MiraAccessibilityService requireAccessibility() throws PermissionException {
        MiraAccessibilityService service = MiraAccessibilityService.connectedService();
        if (service == null) {
