...
const expected=['ssh','sshd','sshd-session','sshd-auth','scp','sftp','sftp-server','ssh-keygen'];
...
// Every desktop release exposes one canonical Mira image. OpenSSH entry links
// are private runtime details and Mira's own roles are always explicit args.
const image='mira'+(platform==='windows'?'.exe':'');
fs.copyFileSync(path.join(source,manifest.image),path.join(destination,image));
fs.cpSync(path.join(source,'licenses'),path.join(destination,'licenses'),{recursive:true});
fs.writeFileSync(path.join(destination,'openssh.json'),JSON.stringify({...manifest,image},null,2)+'\n');
fs.chmodSync(path.join(destination,image),0o755);
if(platform==='linux'){
  for(const role of expected)if(role!==image)fs.symlinkSync(image,path.join(destination,role));
