...
func (client *controlClient) runSSHWorker(ctx context.Context, message controlMessage) error {
	...
	command := backgroundCommand(exec.Command(executable, "ssh-worker"))
	...
	defer close(done)
	if err := json.NewEncoder(input).Encode(sshWorkerConfig{Token: client.token, ClientPublicKey: message.ClientPublicKey, Roots: client.runtime.roots, StateDirectory: filepath.Dir(client.configuration.IdentityFile)}); err != nil {
		stop()
		_ = command.Process.Kill()
		_ = command.Wait()
		return err
