...
func (runtime *capabilityRuntime) process(ctx context.Context, params processParams) (any, error) {
	...
	case "count":
		...
		managedRunning, managedRetained := runtime.managedProcessCounts()
		return map[string]any{
			"processCount":    count,
			"systemVisible":   count,
...
func (runtime *capabilityRuntime) managedProcessCounts() (running int, retained int) {
...
func (process *managedProcess) wait() {
	...
	if err != nil {
		process.signal = processExitSignal(err)
	}
	if err != nil && process.signal == "" {
...
func (runtime *capabilityRuntime) managedProcess(id string) (*managedProcess, error) {
	...
	if process == nil {
		return nil, fmt.Errorf("managed process not found")
	}
	return process, nil
}
