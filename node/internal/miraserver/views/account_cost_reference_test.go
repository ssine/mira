...
func (service *Service) scanAccountCostHistory(ctx context.Context, name, rangeName, zone string) (map[string]any, error) {
	...
	for {
		...
		for _, value := range page {
			...
			err := service.scanAccountCostRecords(ctx, value.store, value.thread, state, name, providers, from, now, func(at time.Time, delta costTotals) {
...
func (service *Service) scanAccountCostRecords(ctx context.Context, store string, thread Thread, state *CostProjection, name string, providers []string, from, to time.Time, consume func(time.Time, costTotals)) error {
	cursor := int64(0)
